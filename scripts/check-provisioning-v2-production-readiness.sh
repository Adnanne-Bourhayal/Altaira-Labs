...
PREPARE_SCRIPT="$ROOT_DIR/scripts/prepare-provisioning-v2-production-checks.sh"
...
MODE="local"
CONFIG_FILE="${ALTAIRA_PRODUCTION_READINESS_ENV:-}"

fail() {
  echo "READINESS ABORT: $*" >&2
  exit 1
}
...
load_config() {
  ...
  [[ -n "$CONFIG_FILE" ]] || fail "ALTAIRA_PRODUCTION_READINESS_ENV is required for a database check"
  ...
  while IFS='=' read -r key value || [[ -n "$key" ]]; do
    ...
    case "$key" in
      SPRING_DATASOURCE_URL|SPRING_DATASOURCE_USERNAME|SPRING_DATASOURCE_PASSWORD|ALTAIRA_EXPECTED_PRODUCTION_HOST|ALTAIRA_PRODUCTION_READ_ONLY_CONFIRMATION)
...
if [[ "$MODE" == "local" ]]; then
  echo "PASS: local-only readiness completed; no database connection was attempted."
  exit 0
fi
...
for required in \
  ...
  ALTAIRA_PRODUCTION_READ_ONLY_CONFIRMATION; do
...
[[ "$ALTAIRA_PRODUCTION_READ_ONLY_CONFIRMATION" == "I_CONFIRM_READ_ONLY_PRODUCTION_PREFLIGHT" ]] \
  || fail "invalid read-only production confirmation"
...
echo "=== EXPLICIT READ-ONLY PRODUCTION $MODE ==="
...
PGAPPNAME="altaira_production_readiness_read_only" \
...
echo "PASS: read-only $MODE completed. No migration was executed."
