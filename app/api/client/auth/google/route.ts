...
export async function POST(request: Request) {
  ...
  try {
    ...
    return invalidJsonResponse("Google login request must be valid JSON.")
  }

  if (!body || typeof body !== "object" || Array.isArray(body)) {
    return invalidJsonResponse("Google login request must include a credential.")
  }

  const { credential } = body as { credential?: string }

  try {
    const response = await fetch(backendUrl("/api/v1/auth/client/google"), {
      method: "POST",
      headers: {
    ...
    return backendServiceUnavailableResponse("Client Google auth service")
  }
}
