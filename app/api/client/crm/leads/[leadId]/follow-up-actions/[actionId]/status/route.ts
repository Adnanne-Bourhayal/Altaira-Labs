...
type RouteContext = {
  params: Promise<{ leadId: string; actionId: string }>
}
...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  const { leadId, actionId } = await context.params

  try {
    const response = await fetch(backendUrl(`/api/v1/client-crm/client/leads/${leadId}/follow-up-actions/${actionId}/status`), {
      method: "PATCH",
    ...
    return backendServiceUnavailableResponse("Client CRM follow-up action service")
  }
}
