...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    return invalidJsonResponse("CRM follow-up action request must be valid JSON.")
  }

  const { leadId } = await context.params

  try {
    const response = await fetch(backendUrl(`/api/v1/client-crm/client/leads/${leadId}/follow-up-actions`), {
      method: "POST",
