...
export async function GET(_: Request, context: RouteContext) {
  ...
  try {
    const response = await fetch(backendUrl(`/api/v1/client-crm/admin/clients/${id}/webhook-tokens`), {
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    return invalidJsonResponse("CRM webhook token request must be valid JSON.")
  }
